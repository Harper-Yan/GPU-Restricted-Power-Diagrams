...
inline int constexpr H = 1024;
inline int constexpr warp_size = 32;
const int DIM = 3;
...
__global__
void BucketSortPoints(int n, const float* data, float* d_data, const int* bin_ids, int* d_cell_psum,  int* d_sorted_idx) {
...
void Calculate_Distances(
    ...
    int* __restrict__ hub_counts,
    int* __restrict__ dH_assignments   
)
{
    ...
    int local_idx = threadIdx.x + blockIdx.x * blockDim.x;
    int global_idx = local_idx + b_id * b_size;
...
template <class R>
__global__
void BucketSort(int n, R * arr_x, R *arr_y, R *arr_z, int * arr_idx, R const* points, int const* dH_assignments, int * dH_psum )
{
    ...
    if( idx < n )
    {
        int const hub_idx = dH_assignments[idx];
        int const loc = atomicAdd(&dH_psum[hub_idx], 1);
 ...
 */
__global__
void Construct_D( float const * distances, int const * assignments, int b_id, int b_size, int n, float * D )
{
    ...
    for( int p = block_level_lane_id; p < b_size; p += blockDim.x )
    {
        int idx = b_id * b_size + p;
        if( idx < n)
        {
            int const this_H = assignments[ idx ];
...
void fused_transform_sort_D( R     const * D // square matrix with lower dist bound from hub i to j
                           , int       * sorted_hub_ids  // square matrix where (i,j) is id of j'th closest hub to i
                           ...
                           )
{
    ...
    int const     lane_id   = threadIdx.x;
    int const     warp_id   = threadIdx.y;
    int const     sort_id   = warp_id * warp_size + lane_id;
    int const     hub_id    = blockIdx.x;
    ...
    for( int coop = warp_size << 1; coop <= H; coop = coop << 1 )
    {
        // do first steps of merge in shared memory 
        for( int stride = coop >> 1; stride >= warp_size; stride = stride >> 1 )
        {
            for ( int r = 0; r < ROUNDS; r ++)
            {
            ...
            for ( int r = 0; r < ROUNDS; r ++)
            {
                ...
                int paired_thread = (global_lane_id)  ^ stride;
                R     const paired_dist = smem[ paired_thread ];
                int const paired_hub  = int(smem[ paired_thread + H ]);
...
__device__ int get_assignment_another_cell(const float q_xyz[3], int cell, int** dH, float** data) {
...
)
{
    ...
    const int* arr_idx = arr_idx_arr[cell];
    const float* arr_x  = arr_x_arr[cell];
    const float* arr_y  = arr_y_arr[cell];
    const float* arr_z  = arr_z_arr[cell];
    const int* psum         = dH_psum[cell];

    int scan_hub_from = psum[hub_containing_qp];
    int scan_hub_to   = psum[hub_containing_qp + 1];
    ...
    int const lane_K  = (K - 1) % warp_size;
    int const round_K = ROUNDS - 1;
    
    #pragma unroll
    for (int r = 0; r < ROUNDS; ++r) {
        int idx = lane_id + r * warpSize;
        int const* hub_points = arr_idx + scan_hub_from;
...
void Query( int stream_id, int K, int* __restrict__ cell_bin_counts,
    ...
    int* __restrict__ solutions_knn,
    int** __restrict__ arr_idx_arr,
    int* __restrict__ sorted_global_idx,
...
)
{
    ...
    int * global_idx_current_cell = sorted_global_idx + cell_psum[cell];
...
template <class R>
void C_and_Q(std::size_t n, R *data, std::size_t q, int *queries, std::size_t k, int *  best_p, int *results_knn, R *results_distances, std::string& mesh_name, float * time_ms)
{
    int constexpr block_size = 1024;

    //---------Looking for best paritition cell grid and its execution time ----------
    // float best_total = std::numeric_limits<float>::infinity();
    // int   best_p_val = -1;

    // ---- CSV setup (create dir, open file, write header if new) ----
    // namespace fs = std::filesystem;
    // const fs::path log_dir = fs::path("..") / "experiment_data" / "grid_partition" / "hubs_grid" ;
    // fs::create_directories(log_dir);
    // const fs::path csv_path = log_dir / (mesh_name + "_detailLog.csv");
    // const bool file_exists = fs::exists(csv_path);
    // std::ofstream csv(csv_path, std::ios::app);
    // if (!csv) {
    //     std::cerr << "Failed to open CSV log: " << csv_path << "\n";
    // } else if (!file_exists) {
    //     csv << "mesh_name,p,n,k,dim,sections,construction_ms,query_ms,total_ms\n";
    // }
    ...
    for ( int p = 1; p <= 1; p++)
    {
        ...
        int* d_sorted_idx;
        cudaMalloc(&d_sorted_idx, sizeof(int) * n);
        ...
        SAFE_CUDA_FREE(d_cell_psum_copy);
        ...
        int* idx_chunks[num_streams], *results_knn_chunks[num_streams];    
        ...
        //-----------Memory Allocations
        int* dH[num_streams];
        int* dH_psum[num_streams];
        int* dH_psum_copy[num_streams];
        int* d_psum_placeholder[num_streams];
        int* dH_assignments[num_streams];

        float* distances_arr[num_streams];
        float* arr_x_arr[num_streams], *arr_y_arr[num_streams], *arr_z_arr[num_streams];
        int* arr_idx_arr[num_streams];
        float* D_arr[num_streams];
        int* iD_arr[num_streams];
        ...
        for (int i = 0; i < num_streams; ++i) {
            ...
            CUDA_CALL(cudaMalloc((void**)&dH[i], sizeof(int) * H));
            CUDA_CALL(cudaMalloc((void**)&dH_psum[i], sizeof(int) * (H + 1)));
            CUDA_CALL(cudaMalloc((void**)&dH_psum_copy[i], sizeof(int) * (H + 1)));
            CUDA_CALL(cudaMalloc((void**)&d_psum_placeholder[i], sizeof(int) * (H + 1)));

            CUDA_CALL(cudaMalloc((void**)&dH_assignments[i], sizeof(int) * n_stream));

            CUDA_CALL(cudaMemsetAsync(dH_psum[i], 0, sizeof(int) * (H + 1), streams[i]));
            CUDA_CALL(cudaMemsetAsync(dH_psum_copy[i], 0, sizeof(int) * (H + 1), streams[i]));
            CUDA_CALL(cudaMemsetAsync(d_psum_placeholder[i], 0, sizeof(int) * (H + 1), streams[i]));
            CUDA_CALL(cudaMemsetAsync(dH_assignments[i], 0, sizeof(int) * n_stream, streams[i]));
            ...
            CUDA_CALL(cudaMalloc((void**)&arr_idx_arr[i], sizeof(int) * n_stream));

            CUDA_CALL(cudaMalloc((void**)&D_arr[i], sizeof(float) * H * H));
            CUDA_CALL(cudaMalloc((void**)&iD_arr[i], sizeof(int) * H * H));
        ...
        for (int i = 0; i < num_streams; ++i) {
            ...
            int batch_number = (n_stream + batch_size -1) / batch_size;
            int batch_id;
            ...
            SAFE_CUDA_FREE( distances_arr[i] );
            /*
            int* h_dH = new int[H];
            float* h_D_arr = new float[H * H];

            CUDA_CALL(cudaMemcpy(h_dH, dH[i], sizeof(int) * H, cudaMemcpyDeviceToHost));
            CUDA_CALL(cudaMemcpy(h_D_arr, D_arr[i], sizeof(float) * H * H, cudaMemcpyDeviceToHost));

            std::ofstream file_dH("log_dH_stream_" + std::to_string(q) + ".txt");
            for (int j = 0; j < H; ++j)
                file_dH << h_dH[j] << "\n";
            file_dH.close();

            std::ofstream file_D("log_D_arr_stream_" + std::to_string(q) + ".txt");
            for (int row = 0; row < H; ++row) {
                for (int col = 0; col < H; ++col)
                    file_D << h_D_arr[row * H + col] << " ";
                file_D << "\n";
            }
            file_D.close();*/
        ...
        for (int i = 0; i < num_streams; ++i) {
            ...
            cudaMemcpy(d_psum_placeholder[i], dH_psum_copy[i], (H + 1 )* sizeof(int), cudaMemcpyDeviceToDevice);

            cudaMemcpy(dH_psum_copy[i] + 1, d_psum_placeholder[i], H * sizeof(int), cudaMemcpyDeviceToDevice);
            cudaMemcpy(dH_psum[i] + 1, d_psum_placeholder[i], H * sizeof(int), cudaMemcpyDeviceToDevice);
            cudaMemset(dH_psum[i], 0, sizeof(int));
            cudaMemset(dH_psum_copy[i], 0, sizeof(int));
        ...
        for (int i = 0; i < num_streams; ++i) {
            ...
            SAFE_CUDA_FREE(D_arr[i]); 
            /*
            float* h_dD = new float[H * H];
            int* h_iD = new int[H * H];

            // Copy device to host
            CUDA_CALL(cudaMemcpy(h_dD, dD_arr[i], sizeof(float) * H * H, cudaMemcpyDeviceToHost));
            CUDA_CALL(cudaMemcpy(h_iD, iD_arr[i], sizeof(int) * H * H, cudaMemcpyDeviceToHost));

            // Generate filenames
            std::string fname_dD = "dD_" + std::to_string(q) + ".txt";
            std::string fname_iD = "iD_" + std::to_string(q) + ".txt";

            // Open files
            std::ofstream file_dD(fname_dD);
            std::ofstream file_iD(fname_iD);

            if (file_dD.is_open() && file_iD.is_open()) {
                for (int row = 0; row < H; ++row) {
                    for (int col = 0; col < H; ++col) {
                        file_dD << h_dD[row * H + col] << " ";
                        file_iD << h_iD[row * H + col] << " ";
                    }
                    file_dD << "\n";
                    file_iD << "\n";
                }
                ...
                std::cerr << "Failed to open log files for writing.\n";
            }
        ...
        //----------------Pointer Transfer-----------------------
        int **d_dH, **d_dH_psum, **d_dH_assignments;
        ...
        cudaMalloc(&d_dH, sizeof(int*) * num_streams);
        cudaMalloc(&d_dH_psum, sizeof(int*) * num_streams);
        cudaMalloc(&d_dH_assignments, sizeof(int*) * num_streams);
        ...
        cudaMalloc(&d_arr_idx_arr, sizeof(int*) * num_streams);
        cudaMalloc(&d_data_chunks, sizeof(float*) * num_streams);

        cudaMalloc(&d_D_arr, sizeof(float*) * num_streams);
        cudaMalloc(&d_iD_arr, sizeof(int*) * num_streams);
        cudaMalloc(&d_dD_arr, sizeof(float*) * num_streams);

        cudaMemcpy(d_dH, dH, sizeof(int*) * num_streams, cudaMemcpyHostToDevice);
        cudaMemcpy(d_dH_psum, dH_psum, sizeof(int*) * num_streams, cudaMemcpyHostToDevice);
        cudaMemcpy(d_dH_assignments, dH_assignments, sizeof(int*) * num_streams, cudaMemcpyHostToDevice);
        ...
        cudaMemcpy(d_arr_idx_arr, arr_idx_arr, sizeof(int*) * num_streams, cudaMemcpyHostToDevice);
        ...
        cudaMemcpy(d_iD_arr, iD_arr, sizeof(int*) * num_streams, cudaMemcpyHostToDevice);
        ...
        //-------------Work Log---------------
        // cudaMemcpy(hubsScanned.data(),  d_hubsScanned,  n * sizeof(int), cudaMemcpyDeviceToHost);
        // cudaMemcpy(pointsScanned.data(), d_pointsScanned, n * sizeof(int), cudaMemcpyDeviceToHost);
        
        // const std::string base_name = fs::path(mesh_name).stem().string(); // without extension
        // const std::string out_name  = base_name + "_grid_" + std::to_string(p)+"_H_" + std::to_string(H) + ".txt";
        // const fs::path out_path     = log_dir / out_name;

        // std::ofstream fout(out_path);
        // if (!fout) {
        //     std::cerr << "Error: cannot open work log for writing at " << out_path << "\n";
        // } else {
        //     // Header with filename and H
        //     fout << "file=" << mesh_name << ", H=" << H << "\n";
        //     for (std::size_t j = 0; j < n; ++j) {
        //         fout << "idx " << j << ": hubsScanned=" << hubsScanned[j]
        //             << ", pointsScanned=" << pointsScanned[j] << "\n";
        //     }
        //     fout.flush();
        // }
        // time_section(section_times, "Log", section_names, start, stop);
        
        //------End of Work log
        //-----------------Memory Free------------------------
        for (int i = 0; i < num_streams; ++i) {
            SAFE_CUDA_FREE(dH[i]);
            SAFE_CUDA_FREE(dH_psum[i]);
            SAFE_CUDA_FREE(dH_assignments[i]);
            SAFE_CUDA_FREE(arr_x_arr[i]);
            SAFE_CUDA_FREE(arr_y_arr[i]);
            SAFE_CUDA_FREE(arr_z_arr[i]);
            SAFE_CUDA_FREE(arr_idx_arr[i]);
            SAFE_CUDA_FREE(iD_arr[i]);
            SAFE_CUDA_FREE(dD_arr[i]);

            cudaStreamDestroy(streams[i]);
        }

        SAFE_CUDA_FREE(d_dH);
        SAFE_CUDA_FREE(d_dH_psum);
        SAFE_CUDA_FREE(d_dH_assignments);
        SAFE_CUDA_FREE(d_arr_x_arr);
        SAFE_CUDA_FREE(d_arr_y_arr);
        SAFE_CUDA_FREE(d_arr_z_arr);
        SAFE_CUDA_FREE(d_D_arr);
        SAFE_CUDA_FREE(d_iD_arr);
        SAFE_CUDA_FREE(d_dD_arr);
        SAFE_CUDA_FREE(d_hubsScanned);
        SAFE_CUDA_FREE(d_pointsScanned);
        SAFE_CUDA_FREE(d_min_vals_int);
        SAFE_CUDA_FREE(d_max_vals_int);
        SAFE_CUDA_FREE(d_min_vals);
        SAFE_CUDA_FREE(d_max_vals);
        SAFE_CUDA_FREE(d_bin_ids);
        SAFE_CUDA_FREE(d_bin_counts);
        SAFE_CUDA_FREE(d_cell_psum);
        SAFE_CUDA_FREE(d_data);
        SAFE_CUDA_FREE(d_sorted_idx);
        SAFE_CUDA_FREE(d_cell_mins);
        SAFE_CUDA_FREE(d_cell_maxs);
        ...
        // sections_ss << std::fixed << std::setprecision(3);
        // sections_ss << H << ';';
        // for (size_t i = 0; i < section_names.size(); ++i) {
        //     if (i) sections_ss << ';';
        //     sections_ss << section_names[i] << ':' << section_times[i];
        // }
        // const std::string sections_cell = sections_ss.str();

        // csv << mesh_name << ',' << p << ',' << n << ',' << k << ',' << 3 /*dim*/
        //     << ',' << sections_cell
        //     << ',' << std::fixed << std::setprecision(3) << construction
        //     << ',' << std::fixed << std::setprecision(3) << query
